...
const ModalUpdateElectricity = ({ visible, onClose, id, onSuccess }) => {
    ...
    const updateElectricityRoom = (values) => {
        Modal.confirm({
            title: 'Xác nhận cập nhật chỉ số điện',
            content: 'Bạn có chắc chắn muốn lưu các thay đổi cho chỉ số điện này không?',
            okText: 'Cập nhật',
            cancelText: 'Hủy',
            onOk: async () => {
                setLoading(true);
    ...
    return (
        ...
        >
            <Form
                form={form}
                layout="vertical"
                onFinish={updateElectricityRoom}
